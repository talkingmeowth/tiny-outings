...
async function loadExistingActivities(env) {
  ...
  for (let offset = 0; ; offset += 1000) {
    // `data_source` is a display grouping, so use the stable importer name
    // when matching records from a Happity schedule refresh.
    const response = await fetch(`${url}/rest/v1/activities?select=activity_id,source_url,website,google_photo_url,image_url,lat,long&source_name=eq.Happity&limit=1000&offset=${offset}`, {
...
function imageTuple(schedule) {
  return `(${sql(schedule.sourceUrl)}, ${sql(schedule.imageUrl)})`;
}
...
async function main() {
  ...
  const matchedWithImages = matched.filter((schedule) => schedule.imageUrl);
  ...
  const happityImageSql = matchedWithImages.length
    ? `with schedule_images (source_url, image_url) as (\n  values\n    ${matchedWithImages.map(imageTuple).join(',\n    ')}\n)\nupdate public.activities as activity\nset\n  -- Happity's schedule card is activity-specific, so keep it ahead of a\n  -- generic venue or legacy photo whenever the current snapshot supplies one.\n  image_url = schedule_images.image_url,\n  image_source_url = schedule_images.source_url,\n  updated_at = now()\nfrom schedule_images\nwhere lower(trim(trailing '/' from activity.source_url)) = lower(trim(trailing '/' from schedule_images.source_url));\n`
    : '';
