...
-- Replaces verified generic venue cards with real named and timed sessions.
-- Each replacement inherits the generic centre's Published or Draft status;
-- generic cards without a verified current timetable remain unchanged.
...
legacy_hubs as (
  ...
  where source_name = ${sql(legacySourceName)}
    and (
      ...
      or archive_reason in (${sql(genericReplacementReason)}, ${sql(publishedGenericReplacementReason)})
    )
)
...
set public_listing_status = 'archived',
    archive = true,
    archive_reason = case
      when legacy.public_listing_status = 'published' then ${sql(publishedGenericReplacementReason)}
      else ${sql(genericReplacementReason)}
    end,
