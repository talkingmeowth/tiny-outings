...
async function shortlistActivity(activity, ordinal) {
  ...
  return {
    ...
    google_summary: activity.google_summary || null,
    google_primary_type: activity.google_primary_type || null,
    google_place_id: activity.google_place_id || null,
    website: activity.website || null,
    organiser_website: activity.organiser_website || null,
...
async function prepareReviewSheets() {
  ...
  const output = {
    ...
    instructions: `Review each labelled ${candidateSource} finalist strip with Codex multimodal vision. Use the activity title, description, Google Places summary/type, address, provider website and organiser website as identity evidence. Assess whether the image is accurate for this exact activity, captures its essence, and is high quality. Reject venue-only or conflicting-property images (for example apartments, bedrooms, treatment rooms, offices, or unrelated buildings), logos, icons, posters, text graphics, blurred or visibly pixelated images, and generic images that do not represent the listing. For cafes prefer a clear interior or seating overview, then an identifiable exterior/storefront. For every category prefer a genuine wide view that best explains the activity. Return null when no finalist is reliable. Raw candidate indices are zero-based and must be copied exactly from the labels.`,
    summary,
