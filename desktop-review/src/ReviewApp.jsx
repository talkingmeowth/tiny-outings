...
export default function ReviewApp() {
  ...
  async function decide(decision) {
    ...
    try {
      ...
      setRows((old) => old.map((row) => row.activity_id === detail.activity_id ? { ...row, ...update } : row));
      setDetail((old) => ({ ...old, ...update }));
      setNotice(`${decision} · live card unchanged`);
      const next = filtered.find((row) => row.activity_id !== detail.activity_id && row.decision === 'pending');
