...
function isMissingActivity(activity: Record<string, unknown>) {
  return ![
...
function isMissingPublished(activity: Record<string, unknown>) {
  return activity.public_listing_status === 'published' && isMissingActivity(activity)
}
...
async function targetData(
  ...
  let query = supabase.from('activities')
    .select('activity_id,activity_name,address,postcode,borough,category,description,google_summary,google_primary_type,google_place_id,google_place_uri,google_link,website,organiser_website,source_url,source_name,image_source_url,created_at,public_listing_status,archive,audit_image_status,audit_image_url,audit_image_source_url,audit_image_original_url,audit_image_original_source_field,image_review_ignored_at,image_review_approved_source_field,image_review_approved_url,admin_cover_image_url,reviewed_image_url,use_category_image,scraped_image_url,model_selected_url,model_selected_confidence,model_selected_original_url,model_selected_source_field,model_selected_model,model_selected_model_version,user_image_url,organiser_website_downloaded_image,website_downloaded_image,wikimedia_image_url,website_image_url,listing_image_url,codex_image_candidates,codex_image_search_query,codex_image_searched_at,codex_image_search_model,serpapi_image_candidates,serpapi_image_search_query,serpapi_image_candidates_fetched_at,serpapi_image_vision_status,serpapi_image_vision_candidate_index,serpapi_image_vision_reason,website_image_candidates,website_image_candidates_fetched_at,website_image_vision_status,website_image_vision_candidate_index,website_image_vision_reason')
    .eq('archive', false)
    .in('public_listing_status', ['draft', 'published'])
  ...
  const rows = (data || []).map((activity) => ({
    ...
    .filter((activity) => !missingOnly || isMissingActivity(activity))
