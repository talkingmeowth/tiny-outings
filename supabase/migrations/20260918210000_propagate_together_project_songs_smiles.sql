-- Together Project Songs & Smiles is one programme at several care homes.
-- Reuse its organiser-page photo only for verified Happity programme listings.
  ...
  select coalesce(
    ...
    case
      when a.data_source = 'Happity'
        ...
        and public.image_review_class_provider_host(a) = 'thetogetherproject.org.uk'
        and btrim(coalesce(a.activity_name, '')) ~* '^the[[:space:]]+together[[:space:]]+project[[:space:]]+'
        and (
          a.activity_name ~* 'songs?[[:space:]]+(and|&)[[:space:]]+smiles'
          or coalesce(a.website, '') ~* 'songs-smiles'
        )
        then 'together_project_songs_and_smiles'
      when a.data_source = 'Happity'
...
as $$
  select case
    ...
    when public.image_review_canonical_group(a) = 'together_project_songs_and_smiles' then
      public.image_review_url_host(p_image->>'source_page_url') = 'thetogetherproject.org.uk'
      and coalesce(p_image->>'source_page_url', '') ~* '/programmes/songs-and-smiles/?$'
...
-- Existing manual approvals agree on this one high-resolution programme photo.
-- The approval function updates active pending siblings and leaves archived rows alone.
do $$
...
begin
  ...
  where activity.archive = false
    ...
    and public.image_review_canonical_group(activity) = 'together_project_songs_and_smiles'
