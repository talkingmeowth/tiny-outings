...
The main-app display hierarchy is admin cover, admin-provided URL, manually reviewed image, desktop-approved model-review image, then user upload, followed by category artwork. Pending, Rejected and Unsure proposals and automatic model/scraper fields are not in the display hierarchy. A later Reject or Unsure removes the mirrored desktop approval. A manually selected category illustration still takes precedence at the reviewed-image position; admin covers and URLs remain above it.
