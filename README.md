...
Every new importer listing also receives at most one SerpAPI Google Images candidate search before image selection. The query includes the exact activity name, a London ward resolved from its postcode, postcode/borough context, category, and venue-oriented terms. Cafe searches explicitly request interiors, seating, and exteriors. The importer atomically records `serpapi_image_search_attempted_at` before the request, then stores every returned image result with all result metadata in `serpapi_image_candidates` and stores the remaining sanitized response metadata in `serpapi_image_search_metadata`. Success, an empty result, and an ambiguous request failure are all durable outcomes, so neither the importer nor desktop review spends a second search for the same activity.

Selection uses a two-stage review so the model never examines all raw images individually. `npm.cmd run activities:images:prepare-codex-review` first applies deterministic metadata, dimensions, aspect-ratio, download-quality, and perceptual-duplicate checks. It keeps 3-5 finalists per activity, renders labelled activity strips, and combines 10 strips into each compact Codex contact sheet. This applies to every activity category; cafe scoring additionally favours high-quality interiors, seating overviews, and informative exteriors. Run `npm.cmd run activities:images:test-codex-review` for the required 10-activity dry run before a larger batch.

Codex visually reviews only the contact-sheet finalists and writes a selected raw candidate index or explicit rejection. Validate a completed file without database writes using `npm.cmd run activities:images:validate-codex-review -- path/to/decisions.json`, then apply it with `npm.cmd run activities:images:apply-codex-review -- path/to/decisions.json`. Each decision is logged in `activity_image_llm_reviews` with the exact candidate-fetch timestamp, model, workflow version, selected candidate or rejection, reason, and outcome. The chosen image is copied into Supabase Storage and written to `scraped_image_url`; no-choice decisions leave it empty rather than forcing an unsuitable image. Downloaded thumbnails and generated sheets are cached locally so interrupted runs resume without repeating work.

After an applied recurring import, the workflow discovers website candidates for the new records, selects every pending website candidate set, makes the one permitted SerpAPI request for each new record without a durable attempt, selects from the stored SerpAPI sets, and reruns the trained automated reviewer against still-unreviewed published and draft activities. Every selection stage is idempotent and uses stored candidates; only first-time discovery performs network pulling.

The card image hierarchy is: `admin_cover_image_url`, `reviewed_image_url`, `user_image_url`, validated `audit_image_url`, audit-passed `scraped_image_url`, `organiser_website_downloaded_image`, `website_downloaded_image`, `model_selected_url` at 70% or greater confidence, `user_uploaded_image_url`, category-permitted `wikimedia_image_url`, `website_image_url`, `listing_image_url`, then illustrated category artwork. A selected category illustration occupies the manual-review priority below an admin cover but still counts as missing in desktop review.
