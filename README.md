...
Every new importer listing also receives one SerpAPI Google Images candidate search before image selection. The query includes the exact activity name, a London ward resolved from its postcode, postcode/borough context, category, and venue-oriented terms. Cafe searches explicitly request interiors, seating, and exteriors. All usable candidates are stored once in `serpapi_image_candidates`; the paid search is not repeated.
...
Codex visually reviews only the contact-sheet finalists and writes a selected raw candidate index or explicit rejection. Validate a completed file without database writes using `npm.cmd run activities:images:validate-codex-review -- path/to/decisions.json`, then apply it with `npm.cmd run activities:images:apply-codex-review -- path/to/decisions.json`. Each decision is logged in `activity_image_llm_reviews` with the exact candidate-fetch timestamp, model, workflow version, selected candidate or rejection, reason, and outcome. The queue excludes candidate sets that already have that durable review, including the original 20 cafe reviews. The chosen image is copied into Supabase Storage and written to `scraped_image_url`; no-choice decisions leave it empty rather than forcing an unsuitable image. Downloaded thumbnails and generated sheets are cached locally so interrupted runs resume without repeating work.

The recurring importer automatically searches and prepares only activities created during that import run; it does not sweep the historical database. Manual backlog preparation is capped at 10 activities by default. Use an explicit `--limit` only when intentionally preparing a larger paced batch.
